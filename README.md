...
- Hover between top-level blocks to reveal **+**. Left-click inserts and focuses an ordinary paragraph, immediately opening its floating preview when enabled. Right-click **+** opens a small menu for a table or table of contents. Moving away, scrolling, Escape or clicking elsewhere closes the menu.
- In Live Preview, hover a block to reveal a faint **−** on its right. Hover the minus to outline the deletion target in pale red; click to delete the whole block. Use the **Undo** notification or Ctrl+Z to restore it. Deleting a heading keeps the following content; deleting a callout or table removes the entire container.
