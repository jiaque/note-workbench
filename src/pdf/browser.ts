...
export async function findBrowser(configured=''):Promise<string>{
  ...
  throw new Error(t("找不到 Edge / Chrome。请安装浏览器，或在 noteWorkbench.pdf.browserPath 中指定浏览器可执行文件。"));
}
...
export async function printPdf(browser:string,url:string,signal?:AbortSignal):Promise<Buffer>{
  if(signal?.aborted)throw new Error(t("已取消导出"));
  ...
  const abort=()=>{reject(new Error(t("已取消导出")));child.kill();};signal?.addEventListener('abort',abort,{once:true});
  const timeout=setTimeout(()=>{reject(new Error(t("PDF 导出超时；请检查图片、图表或浏览器配置。")));child.kill();},90_000);
  child.on('error',reject);child.on('exit',()=>reject(new Error(t("PDF 浏览器进程已退出"))));
  child.stderr?.resume();
  ...
  reader.on('data',(chunk:string)=>{
    ...
    while((end=buffer.indexOf('\0'))>=0){const line=buffer.slice(0,end);buffer=buffer.slice(end+1);if(!line)continue;
      try{const message=JSON.parse(line),item=pending.get(message.id);if(item){pending.delete(message.id);if(message.error)item.reject(new Error(message.error.message));else item.resolve(message.result);}}catch(error){reject(new Error(t("无法读取浏览器返回结果：")+String(error)));}
    }
  });
  ...
  try{
    ...
    if(!ready)throw new Error(t("PDF 内容渲染未完成"));
    const result=await send('Page.printToPDF',{printBackground:true,preferCSSPageSize:true,displayHeaderFooter:false},sessionId);
    const pdf=Buffer.from(result.data,'base64');if(pdf.subarray(0,5).toString()!=='%PDF-')throw new Error(t("浏览器未返回有效 PDF"));return pdf;
  }finally{
