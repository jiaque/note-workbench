...
async function prepare(){
  ...
  for(const media of document.querySelectorAll('audio,video,[data-pdf-src]')){const label=document.createElement('p');label.className='pdf-media-note';label.textContent=media.hasAttribute('data-pdf-src')?t("[PDF 附件：请在原笔记中打开]"):t("[音视频附件：请在原笔记中播放]");media.replaceWith(label);}
  ...
  await Promise.all([...document.images].map(image=>image.decode().catch(()=>{throw new Error(t("图片加载失败：")+(image.alt||t("未命名图片")));})));
  await new Promise<void>(resolve=>requestAnimationFrame(()=>requestAnimationFrame(()=>resolve())));
  window.__notePdfState={ready:true};
}
void prepare().catch(error=>{window.__notePdfState={error:t("PDF 渲染失败：")+String(error)};});
