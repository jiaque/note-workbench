...
import {insertBlock,makeToc,managedTocs,tocUpdates,type TocOptions} from '../shared/authoring';
import {insertionBlocks} from '../shared/block-ranges';
import {t} from '../shared/i18n';

export class BlockInsert {
  private cachedSource='';private cachedNodes:ReturnType<typeof insertionBlocks>=[];private openingSource='';
  ...
  private hover(x:number,y:number){
    ...
    const source=v.state.doc.toString();if(source!==this.cachedSource){this.cachedSource=source;this.cachedNodes=insertionBlocks(source);}const nodes=this.cachedNodes;let best:{at:number;y:number}|undefined;
    const consider=(at:number,cy:number)=>{if(Math.abs(cy-y)<=14&&(!best||Math.abs(cy-y)<Math.abs(best.y-y)))best={at,y:cy};};
    const start=v.coordsAtPos(nodes[0]?.focus??0);if(start&&!nodes[0]?.yaml)consider(0,start.top-7);
    for(let i=0;i<nodes.length;i++){const end=nodes[i].to,next=nodes[i+1]?.focus;const a=v.coordsAtPos(end),b=next===undefined?null:v.coordsAtPos(next);if(a)consider(end,b?(a.bottom+b.top)/2:a.bottom+8);}
