...
try{
  ...
  await wait(150);const beforeAnchor=live.coordsAtPos(anchorSource.indexOf('\n\n'))!,targetHeading=live.coordsAtPos(anchorSource.indexOf('## Trend'))!;
  live.contentDOM.dispatchEvent(new PointerEvent('pointermove',{clientX:live.contentDOM.getBoundingClientRect().left+5,clientY:(beforeAnchor.bottom+targetHeading.top)/2,bubbles:true}));await wait();check(!livePlus.hidden,'plus before anchored heading appears');livePlus.click();check(live.state.selection.main.head<live.state.doc.toString().indexOf('<a id="trend">'),'inserted paragraph precedes anchor');check(live.state.doc.toString().includes('<a id="trend"></a>\n\n## Trend'),'insertion preserves anchor to heading adjacency');liveInsert.close();
