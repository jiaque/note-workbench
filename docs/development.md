...
`pdf/export.ts` 复用安全正文解析及受笔记库边界限制的资源解析。临时 HTTP 服务仅监听 127.0.0.1，使用随机路径令牌，并仅提供本次授权资源及构建后的静态资源；导出结束关闭。`pdf/browser.ts` 通过隔离临时配置的 Chrome/Edge 与 CDP 管道调用 `Page.printToPDF`，不连接用户浏览器会话。无新增运行时 npm 依赖，也不下载 Chromium。

`webview/export.ts` 等待 Mermaid、字体和图片完成，展开折叠内容；打印 CSS 使用白底 A4、表头重复、尽量避免截断表格行。PDF、音视频附件暂用文字提示。资源/图表失败、进程失败、90 秒超时和取消不会被记作导出成功。浏览器路径配置为 machine 级且受工作区信任限制。
