...
const { renderDocument } = renderer;
const root = resolve('dist/webview');
let source = await readFile('test/fixtures/vault/Welcome.md', 'utf8'), version = 1;
const snapshot = operationId => ({ type: 'snapshot', source, version, name: 'Welcome.md', readonly: false, operationId, ...renderDocument(source) });
const server = createServer(async (request, response) => {
  try {
    ...
    if (url.pathname === '/message' && request.method === 'POST') {
      ...
      if (message.type === 'ready') output.push(snapshot());
      if (message.type === 'edit') {
        if (message.baseVersion !== version) output.push({ type: 'conflict', operationId: message.operationId }, snapshot());
        else {
          ...
          version++; output.push(snapshot(message.operationId));
        }
      }
    ...
    response.setHeader('Content-Type', ({ '.js': 'text/javascript', '.css': 'text/css', '.map': 'application/json' })[extname(file)] ?? 'application/octet-stream');
    response.end(await readFile(file));
  } catch (error) { response.writeHead(500).end(String(error)); }
});
server.listen(4317, '127.0.0.1', () => console.log('Preview: http://127.0.0.1:4317 (in-memory sample; not a production dependency)'));
