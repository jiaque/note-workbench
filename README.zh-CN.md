...
- 编辑时，块预览的顶部提供标题、加粗、斜体、删除线、代码、链接、高亮、列表和引用按钮。源码选区会在预览中同步高亮，光标用细线指示；HTML 属性和图形等无法对应文字的位置以轮廓提示。表格单元格有精简的选区工具栏。
- 鼠标移到顶层块之间时出现浅色 **+**。左键直接插入并聚焦普通段落，立即显示预览浮层（需开启块预览）；右键展开插入目录／表格的小菜单。鼠标移远、滚动、按 Escape 或点击其他位置会关闭菜单。
- 实时预览中，悬停块时右侧仅显示浅色 **−**，移到减号上才用淡红色轮廓标出删除范围。点击删除整个块，可通过提示中的**撤销**或 Ctrl+Z 恢复。删除标题保留后续正文，删除折叠卡或表格则移除整个容器。
- 在 `![](...)` 和 `[](...)` 中输入本地路径时可以补全图片／文件。以当前笔记所在目录为基准，文件夹以 `/` 结尾，支持中文文件名和空格。选中文字后粘贴 HTTP(S) 网址，可直接生成链接。
- 生成的目录默认包含 H2–H3，在停止编辑后更新。悬停目录并点击 **⋯** 可以修改级别、开关自动更新、立即更新或删除。笔记菜单的“更新目录”可手动刷新。标题末尾加上 `<!-- nw:toc-ignore -->` 可排除单个标题；代码块与嵌套块中的标题不会进入目录。

目录使用普通 Markdown 链接，并由 `<!-- nw:toc ... -->` 和 `<!-- /nw:toc -->` 注释标记管理范围，不会改写手写导航。目录采用本扩展的标题锚点规则，其他 Markdown 渲染器可能不同；需要自动更新时请保留标记。

设置中的 `noteWorkbench.editor.formatToolbar.enabled`（快捷格式）、`noteWorkbench.editor.previewSelection.enabled`（预览光标／选区）、`noteWorkbench.editor.toc.autoUpdate`（目录自动更新）默认开启。预览高亮与操作按钮不会写入 Markdown 或 PDF。可用[写作辅助样例](example/Writing-helpers.md)体验。
